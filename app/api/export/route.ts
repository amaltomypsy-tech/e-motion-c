...
export async function GET(req: Request) {
  ...
  const responseRows = responses.map((r: any) => {
    ...
    return {
      ...
      displayed_option_order: safe((r.displayedOptionOrder ?? r.displayed_option_order ?? []).join("|")),
      selected_display_position: r.selectedDisplayPosition ?? r.selected_display_position ?? "",
      original_option_id: safe(r.originalOptionId ?? r.original_option_id),
      original_option_text: safe(r.originalOptionText ?? r.original_option_text),
  ...
  wsResponses.columns = [
    ...
    { header: "displayed_option_order", key: "displayedOptionOrder", width: 24 },
    { header: "selected_display_position", key: "selectedDisplayPosition", width: 24 },
    { header: "original_option_id", key: "originalOptionId", width: 18 },
    { header: "original_option_text", key: "originalOptionText", width: 62 },
  ...
  for (const r of responses) {
    ...
    wsResponses.addRow({
      ...
      branch: safe(r.branch ?? r.branchPrimary),
      branchPrimary: safe(r.branchPrimary),
      displayedOptionOrder: safe((r.displayedOptionOrder ?? r.displayed_option_order ?? []).join("|")),
      selectedDisplayPosition: r.selectedDisplayPosition ?? r.selected_display_position ?? "",
      originalOptionId: safe(r.originalOptionId ?? r.original_option_id),
      originalOptionText: safe(r.originalOptionText ?? r.original_option_text),
