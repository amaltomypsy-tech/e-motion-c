...
export async function POST(req: Request) {
  ...
  try {
    const rec = await saveResponse({
      ...
      displayedOptionOrder: Array.isArray(body.displayedOptionOrder) ? body.displayedOptionOrder : undefined,
      selectedDisplayPosition: Number(body.selectedDisplayPosition ?? 0) || undefined,
      originalOptionId: body.originalOptionId,
      originalOptionText: body.originalOptionText,
    ...
    return NextResponse.json({
      ...
      displayedOptionOrder: rec.displayedOptionOrder ?? rec.displayed_option_order,
      selectedDisplayPosition: rec.selectedDisplayPosition ?? rec.selected_display_position,
      originalOptionId: rec.originalOptionId ?? rec.original_option_id,
      originalOptionText: rec.originalOptionText ?? rec.original_option_text,
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({
    ...
    responses: responses.map((response: any) => ({
      ...
      displayedOptionOrder: response.displayedOptionOrder ?? response.displayed_option_order,
      selectedDisplayPosition: response.selectedDisplayPosition ?? response.selected_display_position,
      originalOptionId: response.originalOptionId ?? response.original_option_id,
      originalOptionText: response.originalOptionText ?? response.original_option_text,
